...
def transcribe_file(self, file_path):
        try:
            print(f"Đang đọc file âm thanh từ: {file_path}")
            audio_data, samplerate = sf.read(file_path, dtype='float32')
            print(f"✓ Đọc file thành công. Độ dài: {len(audio_data)/samplerate:.2f} giây, Sample rate: {samplerate} Hz")
        except Exception as e:
            print(f"Lỗi: Không tìm thấy file hoặc không thể đọc file. Chi tiết: {e}")
            return None
        
        print("Bắt đầu phiên âm toàn bộ file...")
        ...
        if full_text:
            print("✓ Phiên âm hoàn tất!")
        else:
            print("Whisper đã xử lý nhưng không tìm thấy văn bản nào trong file.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        transcribed_text = transcriber.transcribe_file(AUDIO_FILE_PATH)

        if transcribed_text:
            print("\n" + "="*50)
            print("KẾT QUẢ PHIÊN ÂM CUỐI CÙNG:")
            print("="*50)
            print(transcribed_text)
            print("="*50)
        else:
            print("\nKhông có kết quả phiên âm.")
